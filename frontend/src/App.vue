...
/**
 * The app opens on a worked example rather than an empty map, so the route
 * scoring is visible immediately. `?demo=0` opens on the plain explore view.
 */
const DEMO_JOURNEY = {
  origin: { id: 'southern-cross', name: 'Southern Cross Station', kind: 'transport', lat: -37.8183, lng: 144.9525 },
  destination: { id: 'parliament', name: 'Parliament House', kind: 'landmark', lat: -37.811, lng: 144.9733 },
}
...
onMounted(async () => {
  ...
  if (new URLSearchParams(location.search).get('demo') !== '0') {
    // Set these only after the sensors land, so the first plan is scored
    // against real crowd numbers instead of an empty map.
